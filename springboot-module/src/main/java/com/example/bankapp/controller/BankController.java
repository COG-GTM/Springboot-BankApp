...
public class BankController {
    ...
    public String registerAccount(@RequestParam String username, @RequestParam String password, Model model) {
        try {
            bankingServiceFacade.registerAccount(username, password);
    ...
    public String deposit(@RequestParam BigDecimal amount) {
        String username = SecurityContextHolder.getContext().getAuthentication().getName();
        Account account = bankingServiceFacade.findAccountByUsername(username);
        bankingServiceFacade.deposit(account, amount);
    ...
    public String withdraw(@RequestParam BigDecimal amount, Model model) {
        ...
        try {
            bankingServiceFacade.withdraw(account, amount);
        } catch (RuntimeException e) {
            model.addAttribute("error", e.getMessage());
            model.addAttribute("account", account);
            return "dashboard";
    ...
    public String transactionHistory(Model model) {
        String username = SecurityContextHolder.getContext().getAuthentication().getName();
        Account account = bankingServiceFacade.findAccountByUsername(username);
        model.addAttribute("transactions", bankingServiceFacade.getTransactionHistory(account));
        return "transactions";
    }
    ...
    public String transferAmount(@RequestParam String toUsername, @RequestParam BigDecimal amount, Model model) {
        String username = SecurityContextHolder.getContext().getAuthentication().getName();
        Account fromAccount = bankingServiceFacade.findAccountByUsername(username);
